"""
Parabolic SAR Trend-Following backtest.
In-Sample: 2019-2022, Out-of-Sample: 2023-2024.
Parameter optimization: af_init, af_step, af_max, adx_filter, adx_threshold, stop_loss.
"""
...
from strategies import ParabolicSARStrategy
...
PARAM_GRID = {
    "af_init":        [0.01, 0.02, 0.03],
    "af_step":        [0.01, 0.02],
    "af_max":         [0.10, 0.20, 0.30],
...
def cross_sectional_aggregate(per_stock_results, strategy_name="Parabolic SAR Trend-Following"):
...
def optimize(stock_data):
    ...
    for combo in combos:
        ...
        # skip redundant adx_threshold when filter is off
        if not params["adx_filter"] and params["adx_threshold"] != PARAM_GRID["adx_threshold"][0]:
            continue
        strategy = ParabolicSARStrategy(**params)
...
def main():
    print("=" * 60)
    print("Parabolic SAR Trend-Following — Backtest")
    print("=" * 60)
    ...
    for period_name, (start, end) in PERIODS.items():
        ...
        strategy = ParabolicSARStrategy(**best_params)
    ...
    out_path = os.path.join(os.path.dirname(__file__), "parabolic_sar_results.json")
