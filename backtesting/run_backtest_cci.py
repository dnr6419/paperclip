"""
CCI Mean Reversion backtest.
In-Sample: 2019-2022, Out-of-Sample: 2023-2024.
Parameter optimization: cci_period, oversold, overbought, sma_filter.
"""
...
from strategies import CCIMeanReversionStrategy
...
PARAM_GRID = {
    "cci_period":  [14, 20, 30],
    "oversold":    [-150.0, -100.0, -80.0],
    "overbought":  [80.0, 100.0, 150.0],
    "sma_filter":  [100, 200, 250],
}


def cross_sectional_aggregate(per_stock_results, strategy_name="CCI Mean Reversion"):
...
def optimize(stock_data):
    ...
    for combo in combos:
        ...
        strategy = CCIMeanReversionStrategy(**params)
...
def main():
    print("=" * 60)
    print("CCI Mean Reversion — Backtest")
    print("=" * 60)
    ...
    if best_params is None:
        ...
        best_params = {"cci_period": 20, "oversold": -100.0, "overbought": 100.0, "sma_filter": 200}
    ...
    for period_name, (start, end) in PERIODS.items():
        ...
        strategy = CCIMeanReversionStrategy(**best_params)
    ...
    out_path = os.path.join(os.path.dirname(__file__), "cci_results.json")
