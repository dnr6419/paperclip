"""
KRX backtest runner: 11 strategies on REAL Korean market data (pykrx CSV).
...
def run_on_universe(strategy, params, stock_data, market_data):
    results = []
    for ticker, df in stock_data.items():
        ...
        try:
            ...
            if "market_close" in sig_params.parameters:
                mc = market_data["close"].reindex(df.index, method="ffill") if market_data is not None else None
...
def main():
    print("=" * 60)
    print("BACKTESTING RUNNER — 11 Strategies, REAL KRX Data (pykrx)")
    print("=" * 60)
    print("\nLoading real KRX market data from CSV...", flush=True)
    all_data = load_real_data()

    kodex_key = "069500"
    kodex = all_data.pop(kodex_key, None)
    kodex_lever = all_data.pop("122630", None)
    print(f"  {len(all_data)} stocks loaded (KODEX 200 as market proxy)")
    ...
    for period_name, (start, end) in PERIODS.items():
        ...
        for k, v in all_data.items():
            ...
            if len(sliced) >= 100:
                stock_data[k] = sliced
        market_proxy = kodex.loc[start:end] if kodex is not None else None
        ...
        for strat_name, (strategy, params) in STRATEGIES.items():
            ...
            results = run_on_universe(strategy, params, stock_data, market_proxy)
    ...
    out_path = os.path.join(os.path.dirname(__file__), "results_krx_real.json")
